...
import { TrendingUp, ChevronDown, ChevronUp, Filter, X, Check, Search } from "lucide-react";
...
export default function ZScoreLineChart({
    ...
    // Sync selectedLines with locations that have data
    useEffect(() => {
        if (locations && locations.length > 0 && data && data.length > 0) {
            ...
            // Show max 5 locations by default
                                ...
                                <div style={{ flex: 1, maxHeight: 280, overflowY: 'auto', padding: 8 }}>
                                    {filteredLocations.map((loc, idx) => {
                                        ...
                                        return (
                                            <button
                                                key={loc.id}
                                                onClick={() => toggleLine(loc.name)}
                                                disabled={!hasDataForLoc}
                                                style={{
                                                    width: '100%',
                                                    display: 'flex',
                                                    alignItems: 'center',
                                                    ...
                                                    border: 'none',
                                                    borderRadius: 8,
                                                    cursor: hasDataForLoc ? 'pointer' : 'not-allowed',
                                                    opacity: hasDataForLoc ? 1 : 0.4,
                                            ...
                                            >
                                                ...
                                                <span style={{ fontSize: 13, color: '#374151', fontWeight: isSelected ? 600 : 400, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                    {loc.name}
                                                </span>
                                                {!hasDataForLoc && (
                        ...
                        return (
                            <span
                                ...
                                style={{
                                    ...
                                    padding: '6px 10px',
                                    background: 'white',
                                    border: `2px solid ${COLORS[locIdx % COLORS.length]}`,
                                    borderRadius: 20,
                                    fontSize: 11,
                                    fontWeight: 600,
                                    color: COLORS[locIdx % COLORS.length],
                                }}
                            >
                                ...
                                >
                                    <X size={12} color={COLORS[locIdx % COLORS.length]} />
                                </button>
                            </span>
                        );
                    })}
                    {selectedLines.length > 8 && (
                        <span style={{ padding: '6px 10px', background: '#e5e7eb', borderRadius: 20, fontSize: 11, fontWeight: 600, color: '#6b7280' }}>
                            +{selectedLines.length - 8} lainnya
                        </span>
                    )}
                </div>
            )}
                    ...
                    <>
                        <ResponsiveContainer width="100%" height={350}>
                            <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                ...
                                <YAxis domain={[-4, 2]} stroke="#6b7280" style={{ fontSize: '12px', fontWeight: 500 }} />
                                <Tooltip
                                ...
                                <ReferenceLine y={0} stroke="#22c55e" strokeWidth={1} strokeDasharray="3 3" />
                                <ReferenceLine y={-2} stroke="#f59e0b" strokeDasharray="5 5" strokeWidth={2} label={{ value: '-2 SD', position: 'right', fontSize: 10, fill: '#f59e0b' }} />
                                <ReferenceLine y={-3} stroke="#ef4444" strokeDasharray="5 5" strokeWidth={2} label={{ value: '-3 SD', position: 'right', fontSize: 10, fill: '#ef4444' }} />
                        ...
                        <div style={{ marginTop: 16, padding: 12, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}>
                            <p style={{ fontSize: 11, color: '#475569', margin: 0 }}>
                                <strong>Interpretasi:</strong> &gt; -1 SD: Normal | -2 to -1 SD: Kurang | &lt; -2 SD: Buruk
                            </p>
                        </div>
                    </>
                )}
