...
interface MapInsidenProps {
    data: Record<string, number>;
    label: string; // "Insiden Total" or "Insiden Baduta"
...
// Thresholds for Incidence Rate (Assumed, can be adjusted)
...
export default function MapInsiden({ data, label, selectedPuskesmas = null }: MapInsidenProps) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const style = useCallback(
        ...
        [data, selectedPuskesmas, normalizeString]
    );
    ...
    const onEachFeature = useCallback(
        ...
        (feature: any, layer: L.Layer) => {
          ...
          <p style="font-size: 11px; color: #64748b; margin: 0;">
            ${label}: <strong style="color: ${getColor(value)}; font-size: 13px;">${value.toFixed(2)}%</strong>
          </p>
        </div>`,
                {
                    sticky: true,
        ...
        <div className="relative">
            <div className="rounded-2xl overflow-hidden border border-slate-200" style={{ height: "500px" }}>
                ...
                >
                    ...
                    />
                    <GeoJSON
                        key={`${label}-${JSON.stringify(data).length}-${selectedPuskesmas}`}
                        data={geojsonData}
                        style={style}
            ...
            <div className="absolute bottom-4 right-4 bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-slate-200 p-4 z-[1000]">
                ...
                <div className="space-y-1">
                    {[
                        { color: "#991b1b", label: "≥ 3%" },
                        { color: "#dc2626", label: "2-3%" },
                        { color: "#f97316", label: "1-2%" },
                        { color: "#fbbf24", label: "0.5-1%" },
