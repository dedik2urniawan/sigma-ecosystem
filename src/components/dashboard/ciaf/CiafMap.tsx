...
interface CiafMapProps {
    data: Record<string, number>; // Risk Score per Village Name
    metric: string; // "risk_score"
    selectedDesa?: string | null;
    selectedPuskesmas?: string | null;
}

const METRIC_LABELS: Record<string, string> = {
    risk_score: "Indeks Risiko Kegagalan Ganda",
};
...
function getColor(value: number): string {
    // Risk Score: (D*3 + C*2 + E*1) / N * 100
    // Higher is worse.
    // Thresholds:
    // > 20: Very High (Dark Red)
    // 15-20: High (Red)
    // 10-15: Moderate (Orange)
    // 5-10: Low (Yellow)
    // < 5: Very Low (Green)
    return value >= 20 ? "#7f1d1d" :
        value >= 15 ? "#dc2626" :
            value >= 10 ? "#f97316" :
                value >= 5 ? "#fbbf24" :
                    "#10b981";
...
function MapZoomController({
    ...
    useEffect(() => {
        ...
        } else if (selectedPuskesmas && selectedPuskesmas !== "all") {
...
export default function CiafMap({ data, metric, selectedDesa = null, selectedPuskesmas = null }: CiafMapProps) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    useEffect(() => {
        ...
        return () => {
            // Cleanup provided by React-Leaflet
        };
    }, []);
    ...
    const style = useCallback(
        ...
        (feature: any) => {
            ...
            if (desaName) {
                ...
                // Look for direct match first, then normalized
                // The data keys are expected to be normalized if generated by our helper, but let's be safe
                ...
                if (selectedDesa && selectedDesa !== "all" && normalizeString(selectedDesa) === normalized) {
                    isSelected = true;
                }
                // Dim desa not in selected puskesmas
                if (selectedPuskesmas && selectedPuskesmas !== "all" && !selectedDesa && puskName && normalizeString(puskName) !== normalizeString(selectedPuskesmas)) {
                    isDimmed = true;
                }
            }
            ...
            return {
                ...
                fillOpacity: isDimmed ? 0.2 : isSelected ? 1 : (selectedDesa && selectedDesa !== "all" ? 0.4 : 0.8),
            };
        },
    ...
    const onEachFeature = useCallback(
        ...
        (feature: any, layer: L.Layer) => {
          ...
          <p style="font-size: 11px; color: #64748b; margin: 0;">
            ${METRIC_LABELS[metric] || "Score"}: <strong style="color: ${getColor(value)}; font-size: 13px;">${value.toFixed(1)}</strong>
          </p>
        </div>`,
                {
                    sticky: true,
        ...
        return (
            <div className="w-full h-[500px] bg-slate-100 rounded-2xl flex items-center justify-center">
                <div className="text-center">
                    ...
                    <span className="text-sm text-slate-400">Memuat peta risiko...</span>
                </div>
            </div>
        );
    }
        ...
        <div className="relative">
            ...
            <div className="absolute bottom-4 right-4 bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-slate-200 p-4 z-[1000]">
                ...
                <div className="space-y-1">
                    {[
                        { color: "#7f1d1d", label: "≥ 20 (Sangat Tinggi)" },
                        { color: "#dc2626", label: "15-20 (Tinggi)" },
                        { color: "#f97316", label: "10-15 (Sedang)" },
                        { color: "#fbbf24", label: "5-10 (Rendah)" },
                        { color: "#10b981", label: "< 5 (Sangat Rendah)" },
                    ].map((item) => (
