...
export default function UploadPage() {
    ...
    const openManageData = async (config: UploadConfig) => {
        setManageConfig(config);
        setLoadingPeriods(true);
        try {
            const { data, error } = await supabase.rpc('get_distinct_periods');
            if (!error && data) {
                setExistingPeriods(data.map((r: any) => r.periode).filter(Boolean));
            } else {
                setExistingPeriods([]);
            }
        } catch {
    ...
    const handleUpload = async () => {
        ...
        try {
                    ...
                    if (dbCol) {
                        if (stringColumns.includes(dbCol)) {
                            ...
                            if (typeof value === 'number') {
                                ...
                                // Format "DD/MM/YYYY" parsing
                                const parts = value.split('/');
                                if (parts.length === 3) {
                                    mapped[dbCol] = `${parts[2]}-${parts[1].padStart(2, '0')}-${parts[0].padStart(2, '0')}`;
                                } else {
            ...
            if (validRows.length === 0) {
                setUploadResult({
                    success: false,
                    message: "Tidak ada baris valid. Pastikan kolom kunci (Tahun/Bulan/Puskesmas atau Periode/NIK) terisi.",
                });
                setUploading(false);
                return;
            }
