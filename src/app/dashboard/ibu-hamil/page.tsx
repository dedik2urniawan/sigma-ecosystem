...
import { IbuHamilRawRecord } from "@/lib/ibuHamilHelper";
import IbuHamilDataQuality from "./components/IbuHamilDataQuality";
import IbuHamilOverview from "./components/IbuHamilOverview";
import IbuHamilDomainView from "./components/IbuHamilDomainView";
import IbuHamilFilterBar from "./components/IbuHamilFilterBar";
import {
    Activity,
    FileCheck2,
    BarChart3,
...
export default function IbuHamilPage() {
    ...
    const [indikatorSubTab, setIndikatorSubTab] = useState<
        "ringkasan" | "anemia" | "suplemen" | "kek" | "anc"
    >("ringkasan");

    // Filter states
    const [mode, setMode] = useState<"bulanan" | "triwulan">("bulanan");
    const [year, setYear] = useState<string>("2026");
    const [periodVal, setPeriodVal] = useState<number>(8); // Default to August (dataset up to August)
    ...
    // Raw records & loading
    const [allRecords, setAllRecords] = useState<IbuHamilRawRecord[]>([]);
    ...
    // Fetch data_ibu_hamil from Supabase using batch range pagination (fetchAll) to bypass 1,000 row limit
    const fetchData = async () => {
        ...
        try {
            ...
            while (true) {
                ...
                if (error) {
                    console.error("Error fetching data_ibu_hamil batch:", error);
                    break;
                }
            ...
            setAllRecords(allData as IbuHamilRawRecord[]);
            if (latestUpload) {
                setLastUpdated(latestUpload);
            }

            // Detect max month in data and set default periodVal
            if (allData.length > 0) {
            ...
            console.error("Error fetching data_ibu_hamil:", err);
            setAllRecords([]);
        } finally {
            setLoading(false);
        }
        ...
        <div className="space-y-6 pb-12">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-purple-500 to-purple-700 flex items-center justify-center shadow-lg shadow-purple-200 text-white">
                        <span className="material-icons-round text-2xl">pregnant_woman</span>
                    </div>
                    <div>
                        ...
                        <p className="text-sm text-slate-500">
                            Monitoring anemia, suplementasi gizi, KEK, pemeriksaan kehamilan (ANC), kualitas data, dan kepatuhan pelaporan.
                        </p>
                ...
                <button
                    onClick={fetchData}
                    disabled={loading}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-purple-600 bg-white border border-slate-200 rounded-xl shadow-2xs hover:bg-slate-50 transition-all disabled:opacity-50"
                >
            ...
            {/* Global Unified Sticky Filter Bar across all tabs (SS2) */}
            <IbuHamilFilterBar
                mode={mode}
                setMode={setMode}
                periodVal={periodVal}
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex bg-slate-50 p-1.5 rounded-2xl flex-wrap gap-1 border border-slate-200/80">
                        {[
                            ...
                            { id: "anemia", label: "Anemia Ibu Hamil", icon: HeartPulse },
                            { id: "suplemen", label: "Suplementasi Gizi (TTD/MMS)", icon: Pill },
                            { id: "kek", label: "KEK & PMT Lokal", icon: Apple },
                            { id: "anc", label: "Pemeriksaan Kehamilan (ANC)", icon: Stethoscope }
                        ].map((t) => {
